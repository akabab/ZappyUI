...
Class Player
{
	...
	public function		__construct($team, Socket $socket, $bin)
	{
		$this->_team = $team;
		$this->_socket = $socket;
		$this->_level = 1;
	...
	{
		if (strpos($this->_bin, "client.php") !== 0 && strpos($this->_bin, "client.php") !== false)
			$php = substr($this->_bin, 0, strpos($this->_bin, "client.php"));
		else
			$php = "./";
		...
		if ($pid == -1)
			...
			exec($php. "bin/php " .$this->_bin. " -n " .$this->_team. " -p " .$this->_socket->getPort());
	}
